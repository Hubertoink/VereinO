...
test('saves a compact booking exactly once and keeps optional fields progressive', async () => {
  ...
  await page.locator('body').dispatchEvent('mousedown')
  await expect(page.getByRole('listbox')).toHaveCount(0)
  await flyout.getByLabel('Datum der Buchung').fill('2026-09-09')
...
test('Buchungen Plus filters, inspects and keeps the original journal available', async () => {
  ...
  const searchedAssignments = plus.locator('.bp-row').filter({ hasText: 'Ausflug Sommerferien' }).locator('.bp-row-assignments')
  await expect(searchedAssignments).toContainText('Sommerprogramm')
  await expect(searchedAssignments).toContainText('FERIEN')
  ...
  const editDialog = page.locator('.quick-add-modal')
  await expect(editDialog.locator('#quick-add-description')).toHaveValue('Ausflug Sommerferien')
  await editDialog.getByRole('button', { name: 'Schließen', exact: true }).click()
