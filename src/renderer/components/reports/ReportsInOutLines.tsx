...
export default function ReportsInOutLines(props: { activateKey?: number; refreshKey?: number; from?: string; to?: string; sphere?: Sphere; paymentMethod?: PaymentMethod; earmarkId?: number; budgetId?: number; primaryClassificationValueId?: number }) {
  ...
  useEffect(() => {
    ...
    setLoading(true)
    Promise.all([
      (window as any).api?.reports.monthly?.({ from: props.from, to: props.to, sphere: props.sphere, paymentMethod: props.paymentMethod, type: 'IN', earmarkId: props.earmarkId, primaryClassificationValueId: props.primaryClassificationValueId, budgetId: props.budgetId }),
      (window as any).api?.reports.monthly?.({ from: props.from, to: props.to, sphere: props.sphere, paymentMethod: props.paymentMethod, type: 'OUT', earmarkId: props.earmarkId, primaryClassificationValueId: props.primaryClassificationValueId, budgetId: props.budgetId })
    ]).then(([inRes, outRes]) => {
      if (cancelled) return
