...
export function useAiAgentWorkflow(input: UseAiAgentWorkflowInput) {
  ...
  const shouldUseAgentRuntime = (userPrompt: string) => {
    ...
    const wantsVereinoData = /(buchung|buchungen|beleg|belege|journal|budget|tag|tags|zweck|zweckbindung|konto|zahlungskonto|bank|mitglied|mitglieder|beitrag|beitraege|rechnung|rechnungen|zahlung|zahlungen|out|in|report|bericht|controlling|auswertung|pdf|csv|xlsx|excel)/.test(normalized)
    const wantsSpecificAgentTask = (wantsWrite || wantsReportExport) && wantsVereinoData && /(mitglied|mitglieder|budget|budgets|zweck|zweckbindung|tag|tags|buchung|buchungen|bank|bankimport|zahlungskonto|konto|beitrag|beitraege|report|bericht|controlling|journal|auswertung|pdf|csv|xlsx|excel)/.test(normalized)
    if (input.hasOpenReviewWorkflow() && !wantsRebook && !wantsSpecificAgentTask) return false
    return wantsReportExport || wantsExploration || (wantsWrite && wantsVereinoData) || (!!agentSessionId && wantsVereinoData)
  }
