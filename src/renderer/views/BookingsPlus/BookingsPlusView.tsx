...
import { IconArrowDown, IconArrowUp, IconArrowsExchange, IconChevronLeft, IconCopy, IconWallet, IconTargetArrow, IconChevronRight, IconFilter, IconPaperclip, IconPencil, IconPlus, IconReceipt2, IconSearch, IconTrash, IconX } from '@tabler/icons-react'
...
export default function BookingsPlusView({ onResetFilters, externalFilters, jumpRevision, flashId, showBookingDraftTabs, bookingDraftTabs = [], onOpenBookingDraft, onCloseBookingDraft, fmtDate, onNewBooking, onNewInvoice, onReviewInvoice, notify, paymentAccounts, budgets, earmarks, tagDefs, allowVoucherDeletion, closedUntil, generalProfile }: Props) {
    ...
    useEffect(() => window.api.organizations.onSwitched(() => {
        setFilters(emptyFilters); setPage(1); setRows([]); setSelected(null); setDetailOpen(false)
    ...
    const selectRow = (row: BookingPlusRow, button?: HTMLButtonElement) => { if (metaEditing) { notify('info', 'Bitte die Bearbeitung zuerst speichern oder abbrechen.'); return }; setSelected(row); setDetailOpen(true); setSimilarTo(null); if (button) lastRowButton.current = button }
    ...
    return <div className="bookings-plus">
        ...
        <section className="bp-workspace">
            <header className="bp-heading"><div><h1>Buchungen Plus</h1></div><label className="bp-search bp-header-search"><IconSearch size={18} /><input type="search" className="input" value={filters.q} onChange={event => update('q', event.target.value)} placeholder="Suche: #ID, Text, Datum …" title="Suche wie im Journal: #ID, Beschreibung, Partner oder Datum, z. B. Juni 2026" aria-label="Buchungen suchen" />{filters.q && <button type="button" className="btn ghost" aria-label="Suche leeren" onClick={() => update('q', '')}><IconX size={16} /></button>}</label><div className="bp-totals" aria-label="Summen aller gefilterten Buchungen"><RecentBookingsDropdown kind="IN" amount={loading || error || invalidRange ? '—' : money.format(income)} payload={payload} revision={revision} invalidRange={invalidRange} fmtDate={fmtDate} onOpenVoucher={row => selectRow(row)} /><RecentBookingsDropdown kind="OUT" amount={loading || error || invalidRange ? '—' : money.format(expense)} payload={payload} revision={revision} invalidRange={invalidRange} fmtDate={fmtDate} onOpenVoucher={row => selectRow(row)} /><div><span>Saldo</span><strong className={income - expense < 0 ? 'bp-negative' : 'bp-positive'}>{loading || error || invalidRange ? '—' : money.format(income - expense)}</strong></div></div></header>
            ...
            <div className="bp-main-columns">
                <section className="bp-panel bp-list" aria-label="Buchungsliste" aria-busy={loading}>
                    ...
                    <div className="bp-list-toolbar"><strong>{total} Buchungen</strong></div>
                    ...
                    {invalidRange ? <p role="alert" className="bp-empty">„Von“ darf nicht nach „Bis“ liegen.</p> : error ? <div role="alert" className="bp-empty">{error}<button className="btn" onClick={refresh}>Erneut versuchen</button></div> : loading ? <p className="bp-empty" role="status">Buchungen werden geladen …</p> : rows.length === 0 ? <div className="bp-empty"><IconReceipt2 size={36} /><h2>Keine Buchungen gefunden</h2><p>Wähle einen anderen Zeitraum oder setze die Filter zurück.</p></div> : <div className="bp-rows">{rows.map(row => <button key={row.id} type="button" className={`bp-row${selected?.id === row.id ? ' is-selected' : ''}`} aria-pressed={selected?.id === row.id} onClick={event => selectRow(row, event.currentTarget)}>
                    ...
                    <nav className="bp-pagination" aria-label="Buchungsseiten"><span>Seite {page} / {Math.max(1, Math.ceil(total / limit))} · {total} Einträge</span><div><button className="btn" disabled={loading || page === 1} onClick={() => setPage(value => value - 1)} aria-label="Vorherige Seite"><IconChevronLeft size={17} /></button><button className="btn" disabled={loading || page * limit >= total} onClick={() => setPage(value => value + 1)} aria-label="Nächste Seite"><IconChevronRight size={17} /></button></div></nav>
                </section>
                ...
                <aside ref={inspector} tabIndex={-1} className={`bp-panel bp-inspector${detailOpen ? ' bp-inspector--open' : ''}${metaEditing ? ' bp-inspector--editing' : ''}`} role={narrow && detailOpen ? 'dialog' : undefined} aria-modal={narrow && detailOpen ? true : undefined} aria-label="Ausgewählte Buchung">
                    ...
                    {selected && metaEditing ? <VoucherInfoModal key={selected.id} embedded initialEditing voucher={selected} onClose={() => setMetaEditing(false)} suspended={!!attachments} eurFmt={money} fmtDate={fmtDate} notify={notify} budgets={budgets} earmarks={earmarks} tagDefs={tagDefs} allowVoucherDeletion={allowVoucherDeletion} onSaveMeta={saveMeta} onOpenAttachments={() => setAttachments(selected)} /> : selected ? <>
                        <div className="bp-inspector-scroll">
                            ...
                            <h2>{selected.description || 'Ohne Beschreibung'}</h2>{selected.counterparty && <p className="bp-dim">{selected.counterparty}</p>}<strong className={`bp-detail-amount ${signedAmount(selected) < 0 ? 'bp-negative' : selected.type === 'IN' ? 'bp-positive' : ''}`}>{signedMoney.format(signedAmount(selected))}</strong>
