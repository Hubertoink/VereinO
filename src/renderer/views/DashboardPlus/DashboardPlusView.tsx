...
type Props = { loadFinancialMonths?: (today: string) => Promise<MonthRow[]>; generalProfile?: boolean; today: string; onGoToBookings: () => void; onGoToInvoices: () => void; onGoToMembers: () => void; onGoToBudgets: () => void; onGoToBindings: () => void; onGoToAI: () => void; onGoToVoucher: (args: { voucherId: number; recordDate?: string | null }) => void }
...
export default function DashboardPlusView({ loadFinancialMonths, today: initialToday, generalProfile = false, onGoToBookings, onGoToInvoices, onGoToMembers, onGoToBudgets, onGoToBindings, onGoToAI, onGoToVoucher }: Props) {
  const financialOnly = !!loadFinancialMonths
  ...
  useEffect(() => {
    ...
    setLoading(true)
    setError(false)
    if (loadFinancialMonths) {
      loadFinancialMonths(today).then(months => {
        if (alive) setData({ months, accounts: [], opening: 0 })
    ...
    return () => { alive = false }
  }, [today, revision, loadFinancialMonths])
  ...
  return <section className="dashboard-plus" aria-label="Dashboard">
    ...
    <div className="dp-navigation"><div className="dp-segments" role="tablist" aria-label="Dashboardansicht" onKeyDown={event => {
      const tabs: Array<'overview' | 'activity'> = financialOnly ? ['overview'] : ['overview', 'activity']
    ...
    {error ? <div className="dp-state" role="alert"><h2>Dashboard konnte nicht geladen werden</h2><p>Bitte versuche es erneut.</p><button className="btn" onClick={() => setRevision(value => value + 1)}>Erneut versuchen</button></div> : !data || !selected || !current || (!snapshot && !financialOnly) ? <div className="dp-state" role="status">Dashboard wird geladen …</div> : <div id="dp-content" role="tabpanel" aria-labelledby={`dp-${tab}-tab`} aria-busy={loading}>
      ...
      {!financialOnly && <StackedMonthlyCard from={rangeFrom} to={today} months={months.map(row => row.month)} general={generalProfile} revision={revision} />}
      {!financialOnly && <DashboardInsights from={rangeFrom} to={today} revision={revision} onBudgets={onGoToBudgets} onBindings={onGoToBindings} />}
      </>}
      {snapshot && <DashboardAssistant context={JSON.stringify({ from: rangeFrom, to: today, months, membersAsOfToday: snapshot.members, invoicesAsOfToday: snapshot.invoices, activeBudgets: snapshot.activeBudgets })} onSetup={onGoToAI} />}
      <p className="dp-footnote">Gebuchte Werte bis {dateLabel(today)}. {!financialOnly && 'Offene Posten sind nicht im Bestand enthalten.'}{previous && ` Vormonatssaldo: ${money.format(previous.net)}.`}</p>
    </div>}
  </section>
}
