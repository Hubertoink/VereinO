...
export function addVoucherBudget(
    voucherId: number,
    budgetId: number,
    amount: number
): { id: number } {
    const info = getDb()
        .prepare('INSERT OR REPLACE INTO voucher_budgets (voucher_id, budget_id, amount) VALUES (?, ?, ?)')
        .run(voucherId, budgetId, amount)
    return { id: Number(info.lastInsertRowid) }
}

export function addVoucherEarmark(
    voucherId: number,
    earmarkId: number,
    amount: number
): { id: number } {
    const info = getDb()
        .prepare('INSERT OR REPLACE INTO voucher_earmarks (voucher_id, earmark_id, amount) VALUES (?, ?, ?)')
        .run(voucherId, earmarkId, amount)
    return { id: Number(info.lastInsertRowid) }
}

export function removeVoucherBudget(assignmentId: number): { id: number } {
    getDb().prepare('DELETE FROM voucher_budgets WHERE id = ?').run(assignmentId)
    return { id: assignmentId }
}

export function removeVoucherEarmark(assignmentId: number): { id: number } {
    getDb().prepare('DELETE FROM voucher_earmarks WHERE id = ?').run(assignmentId)
    return { id: assignmentId }
}

export function getVoucherBudgetTotal(voucherId: number): number {
    const row = getDb()
        .prepare('SELECT COALESCE(SUM(amount), 0) as total FROM voucher_budgets WHERE voucher_id = ?')
...
export function getVoucherEarmarkTotal(voucherId: number): number {
    const row = getDb()
        .prepare('SELECT COALESCE(SUM(amount), 0) as total FROM voucher_earmarks WHERE voucher_id = ?')
        .get(voucherId) as { total: number }
    return row.total
}
