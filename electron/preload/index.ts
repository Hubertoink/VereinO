...
type AsyncApiPath<T> = {
    [Key in keyof T & string]:
        NonNullable<T[Key]> extends AsyncApiMethod
            ? Key
...
type ApiValueAtPath<T, Path extends string> =
...
type RendererMethodAt<Path extends RendererInvokePath> =
    Extract<ApiValueAtPath<RendererApi, Path>, AsyncApiMethod>
type RendererResultAt<Path extends RendererInvokePath> =
    Awaited<ReturnType<RendererMethodAt<Path>>>
...
const rendererApi = {
    window: {
        ...
        isMaximized: () => invokeRaw<{ isMaximized?: boolean }>('window.isMaximized').then(r => !!r.isMaximized),
    ...
    vouchers: {
        ...
        clearAll: () => invokeRaw<RendererResultAt<'vouchers.clearAll'>>('vouchers.clearAll', { confirm: true })
    },
    paymentAccounts: {
    ...
    db: {
        ...
        smartRestore: {
            ...
            apply: (payload: { action: 'useDefault' | 'migrateToDefault' }) => invoke('db.smartRestore.apply', payload)
        },
        location: {
        ...
        onInitFailed: (cb: (info: { message: string }) => void) => {
            const handler = (_event: IpcRendererEvent, info: { message: string }) => { try { cb(info) } catch { } }
    ...
    organizations: {
        ...
        delete: (payload: { orgId: string; deleteData?: boolean }) => invoke('organizations.delete', payload),
        ...
        onSwitched: (cb: (org: { id: string; name: string; dbRoot: string }) => void) => {
            const handler = (_event: IpcRendererEvent, org: { id: string; name: string; dbRoot: string }) => cb(org)
