// CSV parsing shared in behavior with electron/main/services/bankStatementParser.ts.
// Kept within backend rootDir so Docker can compile independently of Electron.
...
function parseCsvRecords(csv: string, delimiter: string): string[][] {
  ...
  for (let index = 0; index < csv.length; index++) {
    ...
    if (char === '"') {
      if (quoted && csv[index + 1] === '"') {
        ...
        if (!quoted && field.trim()) throw new Error('Ungültiges Anführungszeichen in CSV-Feld.')
        if (quoted && index+1 < csv.length && ![delimiter, '\n', '\r', ' ', '\t'].includes(csv[index+1])) throw new Error('Ungültiges Zeichen nach CSV-Anführungszeichen.')
        quoted = !quoted
      }
  ...
  record.push(field.trim())
  if (record.some(Boolean)) records.push(record)
  if (records.length > 5001) throw new Error('Bitte höchstens 5000 CSV-Zeilen pro Import verwenden.')
  return records
}
...
export function parseCsvStatement(buffer: Buffer, mapping?: BankCsvMapping): ParsedBankStatement {
